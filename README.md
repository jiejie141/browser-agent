...
代价也量过 —— 拿 18 次基线的真实轨迹离线复算：

| 对象 | 旧规则 | 新规则 |
|---|---|---|
| 18 次基线 | （原样） | 只有那 3 次**本来就熔断失败**的运行被命中，熔断点相同（18/18/17）；**没有任何一次通过的运行受影响** |
| 强模型 t06 那次 | 熔断点 = **从不存在**（白烧 20 步） | 熔断点 = **第 7 步** |

⚠️ 但要说清楚：**它把"白烧 13 步"变成"第 7 步明确熔断"，并没有让 t06 变通过。**
"加了一个机制"≠"解决了那条任务" —— 要判断 t06 能不能过，得在新规则下真跑一次，
**这一步还没做**（记在 `docs/实验记录.md` 实验 7 的"下一步"里）。
